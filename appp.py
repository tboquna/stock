...
# -------------------------------------------------------------------
    ...
    if target_ticker:
        ...
        if hist_data is not None:
            ...
            st.markdown("### 🤖 系統技術面與策略計畫")
            col_e, col_s, col_t = st.columns(3)
            
            recent_10d_low = hist_data['Low'].tail(10).min()
            stop_loss_price = recent_10d_low * 0.99 
            if (latest_close - stop_loss_price) / latest_close > 0.1: stop_loss_price = latest_close * 0.90
            take_profit_price = latest_close + ((latest_close - stop_loss_price) * 2)
            
            col_e.metric("📍 預計進場價 (現價)", f"{round(latest_close, 2)}")
            col_s.metric("🛡️ 建議停損價", f"{round(stop_loss_price, 2)}", f"{round(((stop_loss_price - latest_close) / latest_close) * 100, 2)} %", delta_color="off")
            col_t.metric("💰 目標停利價", f"{round(take_profit_price, 2)}", f"+{round(((take_profit_price - latest_close) / latest_close) * 100, 2)} %", delta_color="normal")
